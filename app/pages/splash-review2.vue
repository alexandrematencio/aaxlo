...
onMounted(async () => {
  ...
  // power2.in on the shrink (accelerates, no deceleration at end)
  // power2.out on the morph (decelerates, no acceleration at start)
  // Together they form one unbroken curve. Pause only after the A is formed.
  // ══════════════════════════════════════════════════════
  tl.to(bgRect.value, {
  ...
  // Round corners as the shrink finishes
  .to(bgRect.value, {
    borderRadius: '6px',
    duration: 0.25,
    ease: 'power2.in',
  ...
  // Instant swap to SVG morphRect — no gap
  .call(() => {
  ...
  // power2.out = decelerates to a stop (the "landing" half of the curve)
  .to(morphRect.value, {
    morphSVG: {
  ...
  // ── PAUSE: the A holds for a beat before splitting ──
  .to({}, { duration: 0.15 })

  // ══════════════════════════════════════════════════════
  // PHASE 2: CELL DIVISION — after the pause
  ...
  // ══════════════════════════════════════════════════════
  // PHASE 4: "Local Operations" scramble text reveal — tight and punchy
  // ══════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════
  // PHASE 6: × → X and O → glyph morph start at the EXACT same time
  // Use a label so both animations launch from the same timeline position
  // ══════════════════════════════════════════════════════
  ...
  // O morphs to circle — starts at the SAME label
  .call(() => {
  ...
  // Wait for the O morph to finish (longest of the two at 0.5s)
  ...
  <div ref="splash" class="splash-review">
    ...
    <div ref="logoGroup" class="logo-group">
      ...
      >
        ...
        <defs>
          <clipPath id="review2-glyph-clip">
            <circle cx="158.5" cy="18.5" r="18.5"/>
          </clipPath>
        </defs>

        <g ref="morphGroup" opacity="0">
          <g clip-path="url(#review2-glyph-clip)">
