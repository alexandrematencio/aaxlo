<script setup>
/**
 * SplashScreenV5 — « LE MORPH » (variante de V4 « La Forge »)
 *
 * Identique à SplashScreenV4 dans sa mécanique (scroll virtuel scrubbe un morph
 * MorphSVG bloc → lettre, A1 → A2 → X → L → O, puis endgame O → disque → stripes
 * → -45° → glyph orange → collapse header), avec quatre différences UX :
 *
 *  1. Le hint « Scrollez pour forger » + la ligne verticale qui coule sont
 *     remplacés par l'icône souris (cursor-mouse.svg) qui oscille doucement de
 *     haut en bas pour suggérer le scroll.
 *  2. Le readout mono « FORGE — 00% » devient « MORPHING — 00% » et passe
 *     AU-DESSUS du logo (à la distance symétrique du hint sous le logo).
 *  3. Le bouton skip dit « Skip animation » / « Passer l'animation ».
 *  4. Scroll plus lent : à intensité de scroll égale, on révèle 44% là où V4
 *     révélait 50% (budget ×50/44).
 */
...
// Scroll plus lent que V4 (960) : pour une même intensité de scroll, on révèle
...
onMounted(async () => {
  ...
  // Barre collapsée AVANT toute révélation : c'est l'état de départ du wipe.
  gsap.set(logoSvg.value, { scaleX: 0, transformOrigin: '50% 50%' })
  gsap.set(hintEl.value, { autoAlpha: 0, y: 8 })
  // Tous les états initiaux sont posés → on peut révéler le groupe sans flash :
  // il est centré et sa barre est à scaleX 0 (invisible) en attendant l'intro.
  gsap.set(logoGroup.value, { visibility: 'visible' })
  // Le readout (compteur 00%) est déjà à sa place absolue : on le montre.
  gsap.set(readoutEl.value, { visibility: 'visible' })

  const blocks = [block1.value, block2.value, block3.value, block4.value, block5.value]
  const targets = ['#v5-t-a1', '#v5-t-a2', '#v5-t-x', '#v5-t-l', '#v5-t-o']
  ...
  // (états initiaux logoSvg/hint déjà posés plus haut, avant la révélation)
  // ══════════════════════════════════════════════════════
  introTl = gsap.timeline()
  // fromTo explicite (et non from) : la barre est déjà à scaleX 0 (posée plus haut
  // avant la révélation), donc le wipe doit déclarer ses deux bornes — sinon GSAP
  // prendrait le scaleX 0 courant comme destination et n'animerait rien.
  introTl.fromTo(logoSvg.value,
    { scaleX: 0, transformOrigin: '50% 50%' },
    { scaleX: 1, transformOrigin: '50% 50%', duration: 0.5, ease: 'power3.out' })
  introTl.to(hintEl.value, { autoAlpha: 1, y: 0, duration: 0.4, ease: 'power2.out' }, 0.35)

  // ══════════════════════════════════════════════════════
  // PHASE 1 — LE SCROLL MORPHE : lerp input.t → current → ftl.progress()
  // ══════════════════════════════════════════════════════
  tickerFn = () => {
    if (finished || endgameStarted) return
...
<template>
  <div ref="splash" class="splash-v5" role="presentation">
    <button
      ref="skipBtn"
      type="button"
      class="splash-skip"
    ...
    <!-- visibility:hidden inline (pas seulement via CSS scopé) → garanti dès le
         ...
         GSAP révèle (visibility:visible) une fois le centrage + scaleX 0 posés. -->
    <div ref="logoGroup" class="logo-group" style="visibility: hidden" aria-hidden="true">
      <!--
      ...
      >
        <defs>
          <clipPath id="v5-glyph-clip">
            <circle cx="158.5" cy="18.5" r="18.5" />
          </clipPath>
          <!-- Cibles de morph — jamais rendues, mêmes paths que l'original -->
          <path id="v5-t-a1" d="M25.735 29.2004H10.089L7.28289 36.807H0L13.811 0.119141H22.3888L36.1997 36.807H28.5395L25.7333 29.2004H25.735ZM12.1924 23.5881H23.63L18.9905 11.0166L17.912 7.23933L16.8335 11.0166L12.1941 23.5881H12.1924Z" />
          <path id="v5-t-a2" d="M63.2313 29.2004H47.5854L44.7792 36.807H37.4963L51.309 0.119141H59.8868L73.6978 36.807H66.0358L63.2297 29.2004H63.2313ZM49.6904 23.5881H61.128L56.4885 11.0166L55.41 7.23933L54.3315 11.0166L49.6921 23.5881H49.6904Z" />
          <path id="v5-t-x" d="M93.4447 26.1242L91.9335 23.5344L90.3685 26.1242L82.9229 36.807H75.261L88.1562 17.8701L76.1801 0.119141H84.5969L91.0713 9.23701L92.7436 12.0968L94.4159 9.23701L100.781 0.119141H108.388L96.4656 17.7611L109.305 36.8053H100.889L93.443 26.1225L93.4447 26.1242Z" />
          <path id="v5-t-l" d="M113.891 0.119141H121.174V30.9263H138.168V36.807H113.889V0.119141H113.891Z" />
          <path id="v5-t-o" d="M141 18.553C141 7.1401 147.664 0 158.473 0C169.283 0 176 7.1401 176 18.553C176 29.9659 169.283 37.002 158.473 37.002C147.664 37.002 141 29.9659 141 18.553Z" />
        </defs>
    ...
    <!-- Hint scroll : l'icône souris oscille de haut en bas — disparaît dès le geste acquis.
         Caché inline pour ne jamais clignoter avant le style/JS ; révélé par l'intro (autoAlpha). -->
    <div ref="hintEl" class="forge-hint" style="visibility: hidden" aria-hidden="true">
      <img src="/images/cursor-mouse.svg" alt="" class="hint-cursor" width="15" height="24" />
    </div>
  </div>
</template>
...
@media (prefers-reduced-motion: reduce) {
  .splash-v5 { display: none !important; }
}
...
.logo-group {
  ...
  left: 50%;
  z-index: 1;
  /* Caché au premier paint (y compris dans le HTML rendu côté serveur).
     GSAP reste seul maître du transform (centrage xPercent/yPercent:-50 + collapse) ;
     onMounted applique l'état initial PUIS révèle — donc aucun lingot décalé visible. */
  visibility: hidden;
...
/* ── Hint : icône souris qui oscille pour suggérer le scroll ── */
.forge-hint {
  position: absolute;
  ...
  pointer-events: none;
  /* Caché au premier paint — l'intro le révèle via autoAlpha (gsap) à 0.35s.
     Sans ça, le curseur clignote à pleine opacité avant le démarrage. */
  opacity: 0;
  visibility: hidden;
}
...
.hint-cursor {
  ...
  display: block;
  opacity: 0.7;
  animation: cursor-bob 1.5s ease-in-out infinite;
  will-change: transform;
}

/* Oscillation verticale douce — descend puis remonte pour suggérer « scrollez » */
